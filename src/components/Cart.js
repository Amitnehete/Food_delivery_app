...
const Cart = () => {
  ...
  const makePayment = async (price) => {
    ...
    const options = {
      ...
      prefill: {
        name: "Amit Nehete",
        email: "amitnehete1998@gmail.com",
        contact: "8484800292",
      },
      notes: {
        address: "",
            ...
            {items.map((item) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between w-full  gap-2 my-2 "
                >
                  <div className="py-1">
                    {item.veg === "VEG" ? (
                      <img
                  ...
                  <div className="namePrice flex justify-between items-center  w-full px-4">
                    <div className="name w-[40%]">{item?.card?.info?.name}</div>
                    <div className="btn flex justify-between items-center gap-3 py-1 px-3 ">
                      <button
                        onClick={() =>
                          item?.quantity >= 2
                            ? dispatch(decrQuantity(item?.card?.info?.id))
                      ...
                      <button
                        onClick={() =>
                          dispatch(incrQuantity(item?.card?.info?.id))
                        }
                      >
                        +
                      </button>
          ...
          <div className="payment bg-white  w-full p-4 flex flex-col gap-2">
            ...
            <div className="price-det  px-2 flex flex-col gap-3 my-2">
              ...
              <div className="delivery flex justify-between items-center">
                ...
                <div className="price w-20">₹ 40</div>
              </div>
            </div>
            <div className="tax flex justify-between items-center my-2">
              <div className="head">GST and Restaurant Charges</div>
              <div className="price w-24 px-2">₹ 10</div>
            </div>
          </div>
          <div className="mt-2 sticky bottom-0  w-full max-[800px]:bottom-14">
            <button
              className="flex bg-[rgb(249,196,89)] w-full p-3 justify-between items-center text-white text-sm font-bold rounded-md"
              onClick={() => makePayment(totalAmount.current + 50)}
            >
              <div className="rate flex gap-2 items-center">
                PROCEED TO PAY <BsArrowRightSquareFill />
              </div>
              <div className="w-28 p-1 ">
                ₹ {(totalAmount?.current + 50)?.toFixed(2)}
              </div>
            </button>
          </div>
        </div>
